...
<div class="text-center" style="position: absolute; z-index: 999; width: 100%;">
    @if ($message = Session::get('success'))
        <div id="successMessage" class="alert alert-success alert-dismissible fade show bg-green-500 p-4" role="alert">
            <strong>Success!</strong> {{ $message }}
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    @endif
    @if ($message = Session::get('error'))
        <div id="errorMessage" class="alert alert-danger alert-dismissible fade show bg-red-200 p-4" role="alert">
            <strong>Something's wrong!</strong> You should check in on some of those fields below.
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    @endif
    @if ($message = Session::get('warning'))
        <div id="warningMessage" class="alert alert-warning alert-dismissible fade show bg-yellow-500 p-4" role="alert">
            <strong>{{ $message }}</strong>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    @endif
    @if ($message = Session::get('info'))
        <div id="infoMessage" class="alert alert-info alert-dismissible fade show bg-blue-300 p-4" role="alert">
            <strong>{{ $message }}</strong>
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
    @endif
